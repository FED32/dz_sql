...
(extract(second from (return_date - rental_date)))::real/(24.0*60.0*60.0)))::numeric, 2) as "Среднее количество дней на возврат"
from rental
group by customer_id
...
(return_date- rental_date)::interval,
extract(epoch from (return_date - rental_date))/86400,
extract(day from (return_date - rental_date)),
extract(hour from (return_date - rental_date))/24,
extract(minute from (return_date - rental_date))/(24.0*60.0),
extract(second from (return_date - rental_date))
from rental
